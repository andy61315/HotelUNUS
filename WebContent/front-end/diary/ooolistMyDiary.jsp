...
<%
    ...
    List<DiaryVO> diaryList = diarySvc.getMyDiary(customerVO.getCus_Id());
        ...
        <c:forEach var="DiaryVO" items="${diaryList}">
            <div class="card mb-3" style="max-width: 100%;border-radius: 35px;overflow: hidden">
                <div class="row no-gutters">
                    <div class="col-md-4">
<%--                         <a href="<%=request.getContextPath()%>/front-end/diary/diary.do?action=getOne_For_Display&diary_no=${DiaryVO.diary_no}"><img src="<%= request.getContextPath()%>/front-end/diary/diary.do?action=getBlogImg&blog_no=${DiaryVO.diary_no}" class="card-img-top" alt="日誌預覽圖"></a> --%>
                    </div>
...
<%--                                     <span class="text-muted ml-2">觀看次數:${DiaryVO.watch_count}</span> --%>
                                </div>
