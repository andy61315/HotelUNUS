...
<%
	...
	List<DiaryReportVO> diaryReportListStatus2 = diaryReportList.stream()
			.filter(diaryReportVO -> diaryReportVO.getDiary_report_status() == 0).collect(Collectors.toList());

	pageContext.setAttribute("diaryReportListStatus2", diaryReportListStatus2);
%>
...
<title>Audited Hotel Unus</title>
</head>
<body>
								...
								<table class="table table-hover" style="word-break: break-all">
									...
									<tbody>
										...
										<c:forEach var="DiaryReportVO"
											items="${diaryReportListStatus2}" begin="<%=pageIndex%>"
											end="<%=pageIndex+rowsPerPage-1%>">

											<tr>
												<th scope="row">${diarySvc.getAuditedDiary(DiaryReportVO.diary_no).diary_title}</th>
