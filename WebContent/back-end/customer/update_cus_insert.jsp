...
<%
  CustomerVO customerVO = (CustomerVO) request.getAttribute("customerVO"); //EmpServlet.java (Concroller) 存入req的empVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>

<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"/>
<title>會員資料修改 - update_cus_input.jsp</title>
...
<table>
	...
	<tr>
		...
		<td><input type="RADIO" name="cus_Ck" size="45"
			 value="0" <%= (customerVO.getCus_Ck()==0)?"checked":"" %>/><label for="no">未認證</label>
			<input type="RADIO" name="cus_Ck" size="45"
			 value="1" <%= (customerVO.getCus_Ck()==1)?"checked":"" %>/><label for="yes">已認證</label>
			 <input type="RADIO" name="cus_Ck" size="45"
			 value="2" <%= (customerVO.getCus_Ck()==2)?"checked":"" %>/><label for="ghost">註銷</label>
		</td>
	</tr>
	<tr>
		<td>上傳護照</td>
