...
<%@ include file="/back-end/homepage/middle.file" %> 
    			...
    			<div class="row">
    				<div class="col-md-12">
    					<div class="card" style="background-color: white !important; height: 820px;">
    						<div class="card-body">
			    				<!-- NAV BAR --> 
				    			<div class="row">
				    				<div class="col-md-12">
...
$(document).ready(function(){
	...
	$('#demo').bootstrapTable({
	  columns:[ // 欄位設定
	    ...
	    {field:'meal_introduction', title:'餐點介紹', align:'center', width:100, visible:true},
	  ],
	  data : [],
		...
		loadData(param);
			...
			swal({
	    			...
	    			html:
    					...
    					'<img id="mealEditImg" style="width: 250px; height: auto;" />' +
    					'</div>' +
		    			'<form enctype="multipart/form-data" method="POST" name="mealForm">' +
		    			  '<div class="form-group">' +
		    			    '<label for="meal_picture" class="pull-left">' +
		    			    	'餐點相片：' + 
		    			    '</label>' +
		    			    '<input type="file" class="form-control" id="meal_picture" accept="image/*" onchange="changeInputPic()">' +
		    			  '</div>' +
		    			  '<div class="form-group">' +
		    			  ...
		    			  '<div class="form-group">' +
		    			    ...
		    			    '<select id="res_no" class="form-control">' +
			    			    '<option value="REST000001" "' +  ((row.res_no === 'REST000001')?'" selected>':'">') +'辰園</option>' +
			    				'<option value="REST000002" "' +  ((row.res_no === 'REST000002')?'" selected>':'">') +'桃屋</option>' +
			    				'<option value="REST000003" "' +  ((row.res_no === 'REST000003')?'" selected>':'">') +'彼得廳</option>' +
	    					'</select>' +
		    			  '</div>' +
		    			  '<div class="form-group">' +
		    			    '<label for="meal_type_no" class="pull-left">餐點類別：</label>' +
		    			    '<select id="meal_type_no" class="form-control">' +
			    			    '<option value="MT001" "' +  ((row.meal_type_no === 'MT001')?'" selected>':'">') +'主菜</option>' +
			    				'<option value="MT002" "' +  ((row.meal_type_no === 'MT002')?'" selected>':'">') +'甜點</option>' +
			    				'<option value="MT003" "' +  ((row.meal_type_no === 'MT003')?'" selected>':'">') +'湯</option>' +
			    				'<option value="MT004" "' +  ((row.meal_type_no === 'MT004')?'" selected>':'">') +'小菜</option>' +
			    				'<option value="MT005" "' +  ((row.meal_type_no === 'MT005')?'" selected>':'">') +'港式點心</option>' +
	    					'</select>' +
		    			  '</div>' +
		    			  '<div class="form-group">' +
    			...
    			html:
    				...
    				'<form enctype="multipart/form-data" method="POST" name="mealForm">' +
	    			  ...
	    			  '<div class="form-group">' +
	    			    '<label for="res_no" class="pull-left">餐廳名稱：</label>' +
	    			    '<select id="res_no" class="form-control">' +
		    			    '<option value="REST000001">辰園</option>' +
		    				'<option value="REST000002">桃屋</option>' +
		    				'<option value="REST000003">彼得廳</option>' +
  					'</select>' +
	    			  '</div>' +
	    			  '<div class="form-group">' +
	    			    '<label for="meal_type_no" class="pull-left">餐點類別：</label>' +
	    			    '<select id="meal_type_no" class="form-control">' +
		    			    '<option value="MT001">主菜</option>' +
		    				'<option value="MT002">甜點</option>' +
		    				'<option value="MT003">湯</option>' +
		    				'<option value="MT004">小菜</option>' +
		    				'<option value="MT005">港式點心</option>' +
  					'</select>' +
	    			  '</div>' +
	    			  '<div class="form-group">' +
	    			    '<label for="meal_introduction" class="pull-left">餐點介紹：</label>' +
...
function changeInputPic() {
	var file = $('#meal_picture')[0].files[0];
    var reader = new FileReader;
    reader.onload = function(e) {
      $('#mealEditImg').attr('src', e.target.result);
    };
